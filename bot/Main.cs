...
namespace DotaTextGame
{
    class Main
    {
        ...
        public async Task bw_DoWork(string key)
        {
            ...
            try
            {
                ...
                while (true)
                {
                    ...
                    foreach (var update in updates.Where(x => x.Message != null && x.Message.Type == Telegram.Bot.Types.Enums.MessageType.TextMessage))
                    {
                        ...
                        if (_usr.lang.lang == User.Text.Language.NULL && _usr.status != User.Status.LanguageChanging)
                        {
                            _usr.status = User.Status.LanguageChanging;
                            ReplyKeyboardMarkup kb = new ReplyKeyboardMarkup();
                            kb.ResizeKeyboard = true;
                            kb.OneTimeKeyboard = true;
                            kb.Keyboard = new Telegram.Bot.Types.KeyboardButton[][]
                            {
                                new Telegram.Bot.Types.KeyboardButton[]
                                {
                            ...
                            await _usr.Sender.SendAsync(lang => "Choose language:", kb);
                        }
                        ...
                        if (_usr.status == User.Status.Default)
                        {
                            ...
                            {
                                await _usr.Sender.SendAsync(lang => lang.StartMessage);
                            }
